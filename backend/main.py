...
# ============ Тест по технике безопасности в кабинете информатики ============
TB_POINTS_PER = 10       # баллов за верный ответ
TB_PASS_SCORE = 100      # порог допуска к работе за ПК
# Индексы вариантов: а=0, б=1, в=2, г=3
TB_QUIZ = [
    {"q": "Кто допускается к самостоятельной работе в кабинете информатики?",
     "options": ["любой учащийся школы", "только учащиеся, прошедшие инструктаж по ТБ и расписавшиеся в журнале", "только учащиеся с высокими оценками по информатике", "учащиеся с устного разрешения одноклассника"], "correct": 1},
    {"q": "Каким напряжением питается компьютерное оборудование в кабинете?",
     "options": ["12 В", "36 В", "220 В", "1000 В"], "correct": 2},
    {"q": "В чём главная опасность поражения током 220 В для человека?",
     "options": ["лёгкое покалывание без последствий", "только поверхностные ожоги кожи", "поражение сердца, нарушение дыхания, ожоги", "опасно лишь при контакте дольше 10 минут"], "correct": 2},
    {"q": "Какой признак указывает на неисправность электрооборудования и требует немедленно прекратить работу?",
     "options": ["ровный гул вентилятора", "свечение индикатора питания", "искрение, запах гари, нагрев корпуса или вилки", "появление заставки на экране"], "correct": 2},
    {"q": "Каким должно быть ПЕРВОЕ действие, если человека поражает электрический ток?",
     "options": ["облить пострадавшего водой", "обесточить оборудование, не касаясь пострадавшего голыми руками", "сразу оттащить его голыми руками за одежду", "продолжить работу и позвать на помощь на перемене"], "correct": 1},
    {"q": "Каким огнетушителем допустимо тушить электрооборудование под напряжением?",
     "options": ["водным", "воздушно-пенным", "углекислотным или порошковым", "любым, кроме порошкового"], "correct": 2},
    {"q": "Почему электрооборудование под напряжением нельзя тушить водой?",
     "options": ["вода оставляет следы на технике", "вода электропроводна — возможно поражение током", "вода слишком быстро охлаждает технику", "вода усиливает горение пластика"], "correct": 1},
    {"q": "Что относится к наиболее частым причинам возгорания электрооборудования?",
     "options": ["слишком яркое освещение кабинета", "короткое замыкание, перегрузка сети, неисправность техники", "низкая температура в помещении", "частое выключение компьютеров"], "correct": 1},
    {"q": "Каково оптимальное расстояние от глаз до экрана монитора?",
     "options": ["20–30 см", "50–70 см", "90–110 см", "значения не имеет"], "correct": 1},
    {"q": "Как правильно расположить монитор относительно глаз?",
     "options": ["верхний край экрана на уровне глаз или чуть ниже", "центр экрана значительно выше уровня глаз", "экран у самого пола", "экран над головой под углом к столу"], "correct": 0},
    {"q": "Где следует оставлять рюкзаки, сумки и верхнюю одежду в кабинете информатики?",
     "options": ["на рабочем столе рядом с компьютером", "в проходах между рядами", "в специально отведённом месте (стеллаж, полка, зона у входа)", "на клавиатуре или системном блоке"], "correct": 2},
    {"q": "Что разрешается держать на рабочем месте во время урока?",
     "options": ["еду и напитки", "только принадлежности, необходимые для урока", "любые личные предметы в неограниченном количестве", "посторонние тяжёлые предметы"], "correct": 1},
    {"q": "Что запрещено делать во время работы за компьютером?",
     "options": ["сидеть с прямой спиной", "выполнять задания учителя", "трогать разъёмы и заднюю панель под напряжением", "делать перерывы для глаз"], "correct": 2},
    {"q": "Каковы ваши действия при обнаружении оголённого или повреждённого провода?",
     "options": ["поправить его рукой", "заклеить скотчем самостоятельно", "не прикасаться, отойти и немедленно сообщить учителю", "продолжить работу рядом с ним"], "correct": 2},
    {"q": "Что нужно сделать при появлении запаха гари или дыма от техники?",
     "options": ["открыть окно и продолжить работу", "прекратить работу и сообщить учителю", "попытаться самостоятельно разобрать корпус", "полить технику водой"], "correct": 1},
    {"q": "Кто имеет право обесточивать кабинет при аварийной ситуации?",
     "options": ["любой ученик", "дежурный по классу", "учитель (преподаватель)", "староста класса"], "correct": 2},
    {"q": "Кто включает и выключает оборудование в кабинете?",
     "options": ["любой ученик в любое время", "только с разрешения учителя", "дежурный без разрешения", "техника включается автоматически по расписанию"], "correct": 1},
    {"q": "Что относится к мерам электробезопасности?",
     "options": ["работа мокрыми руками", "самостоятельный ремонт техники под напряжением", "работа с исправной техникой и неповреждённой изоляцией", "использование тройников с перегрузкой сети"], "correct": 2},
    {"q": "Что необходимо сделать по окончании работы в кабинете?",
     "options": ["сразу уйти, оставив всё как есть", "сохранить файлы, закрыть программы, навести порядок, выключить технику с разрешения учителя, задвинуть стул", "выключить только монитор", "оставить компьютер включённым для следующего класса"], "correct": 1},
    {"q": "Что нужно сделать перед началом работы за компьютером?",
     "options": ["сразу включить и начать играть", "убедиться в отсутствии видимых повреждений оборудования и проводов", "переставить технику по своему усмотрению", "отключить заземление"], "correct": 1},
]
...
async def tbquiz_submit(init_data: str = Body(...), answers: list = Body(...)):
    tg_user = get_verified_user(init_data)
    correct_key = [q["correct"] for q in TB_QUIZ]
    with get_db() as db:
        me = upsert_user(db, tg_user)
        uid = me["telegram_id"]
        ans = list(answers or [])
